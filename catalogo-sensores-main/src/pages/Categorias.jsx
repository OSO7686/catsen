...
// 🛡️ FUNCIÓN ESCUDO: Limpia símbolos y fuerza el formato de dólares de forma segura
const formatearPrecio = (precio) => {
  if (precio === null || precio === undefined || precio === '') return '$0.00';
  
  // Quitamos cualquier cosa que no sea un número o un punto decimal (elimina $, comas, espacios)
  const textoLimpio = String(precio).replace(/[^0-9.]/g, '');
  const numero = Number(textoLimpio);
  
  // Si la conversión falla, devolvemos el string original asegurando que tenga el signo $
  if (isNaN(numero)) {
    return String(precio).includes('$') ? String(precio) : `$${precio}`;
  }
  
  // Formateamos en dólares (en-US) con sus dos decimales
  ...
  return (
    <div className="min-h-screen bg-white py-12">
      <div className="max-w-7xl mx-auto px-4 flex flex-col lg:flex-row gap-8">
        ...
        {/* BARRA LATERAL CON DISEÑO DE TARJETA AZUL */}
                    ...
                    {estaActiva && (
                      <ul className="mt-1 ml-9 space-y-1 border-l-2 border-blue-100 pl-2 py-2">
                        ...
                        {subsDeEsteMenu.map(sub => {
                          const isSubActivo = subcatActiva === sub.db;
                          return (
                            <li key={sub.db}>
                              <button
                                ...
                                className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                                  isSubActivo ? 'text-blue-600 bg-blue-100/50' : 'text-gray-400 hover:text-blue-500'
                                }`}
                              >
                                • {sub.title}
                              </button>
                ...
                {productosPagina.map((producto, index) => (
                  <div key={index} className="group bg-white border border-gray-100 rounded-3xl p-5 hover:shadow-2xl hover:shadow-blue-100 transition-all duration-500 flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-50">
                      ...
                      {/* 🪙 Aquí se aplica el formateador seguro */}
